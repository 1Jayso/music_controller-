...
class UpdateRoom(APIView):
    ...
    def patch(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if room.host != userid:
                return ({'msg': "You are not the host of this room."}, status=status.HTTP_403_FORBIDDEN)
            room.guest_can_pause = guest_can_pause
            room.votes_to_skip = votes_to_skip
            room.save(update_fields=['votes_to_skip', 'guest_can_pause'])
            return Response(RoomSerializer(room).data, status=status.HTTP_200_OK)

        return Response({'Bad Request': "invlid Data..."}, status=status.HTTP_400_BAD_REQUEST)